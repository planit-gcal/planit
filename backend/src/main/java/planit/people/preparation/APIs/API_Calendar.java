...
public class API_Calendar {
     ...
     *
     * @param planItUserId the PlanIt User Id for whom a new Preset Detail will be
     *                     created.
    ...
    public ResponseEntity<DTO_PresetDetail> createNewPreset(@PathVariable("planit-user-id") Integer planItUserId,
            @RequestBody DTO_PresetDetail dtoNewPreset) {
        try {
     ...
     * Known issue: the time in preset_availability[].end_available_time and
     * .start_available_time in the response, is in the format of HH:mm:ss and not
     * HH:mm as it must be in the request
     * 
     * @param planItUserId the PlanIt User Id for whom we need to get their Preset
     *                     Details
     * @return List of Preset Detail
     */
    @GetMapping(path = "users/{planit-user-id}/presets")
    public ResponseEntity<List<DTO_PresetDetail>> getAllPresetFromPlanItUserId(
            @PathVariable("planit-user-id") Long planItUserId) {
        try {
    ...
    public ResponseEntity<DTO_PresetDetail> updatePreset(@RequestBody DTO_PresetDetail dtoPresetDetail,
            @PathVariable("planit-user-id") Long planItUserId) {
        try {
            System.out.println("request: " + dtoPresetDetail);
            return new ResponseEntity<>(serviceCalendar.upsertPresetDetail(planItUserId, dtoPresetDetail, true),
                    HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
     ...
     *
     * @param planItUserId the id of the PlanIt user whose instance should be
     *                     removed.
    ...
    public ResponseEntity<Boolean> deletePreset(@PathVariable("planit-user-id") Long planItUserId,
            @PathVariable("preset-id") Long presetId) {
        try {
     ...
     * @return HttpStatus.OK if the user is able to join the preset else
